...
if (!vehicleName || !vehicleShortName) {
  setFormError(
    'Lütfen tam araç adı ve kısa adı doldurun.'
  );
  return;
}
...
try {
  ...
  if (
    ...
    setFormError(
      'Yetki / RLS hatası: Bu aileye araç ekleme yetkiniz bulunmuyor.'
    );
  } else {
    setFormError(
...
return (
  ...
  {!vehicles || vehicles.length === 0 ? (
    ...
    <div style={styles.emptyContainer}>
      ...
      <EmptyState
        message="Henüz araç tanımlanmamış"
        icon="🚗"
      />
  ...
  {selectedVehicle && (
    <VehicleDetailModal
      vehicleName={
        selectedVehicle.short_name
      }
      vehicleId={
        selectedVehicle.id
      }
      income={incomeRecords.filter(
        (r) =>
          r.vehicleId ===
          selectedVehicle.id
      )}
      expenses={expenseRecords.filter(
        (r) =>
          r.vehicleId ===
          selectedVehicle.id
      )}
