...
export function HomePage({ familyId }: { familyId: string }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setVehicleSummaries((vehData || []) as VehicleSummaryData[]);
      ...
      setDailySummaries((dailyData || []) as DailySummaryData[]);

      // 4. Notifications Query (Credit Card Unpaid)
  ...
  useEffect(() => {
    ...
    // Realtime 7 Tables Subscription with 500ms Debounce
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.heroCard}>
        ...
        <div style={styles.heroSubGrid}>
          <div>
            ...
            <p style={styles.subSubVal}>${familySummary?.total_income.toLocaleString('en-US', { minimumFractionDigits: 2 }) ?? '0.00'}</p>
          </div>
          <div>
        ...
        <div style={styles.metricsRow}>
          <span>⚡ ${familySummary?.hourly_rate.toFixed(2) ?? '0.00'} / saat</span>
          <span>🚗 ${familySummary?.per_mile_rate.toFixed(2) ?? '0.00'} / mil</span>
          <span>🛣️ {familySummary?.total_miles.toLocaleString() ?? '0'} mil</span>
        </div>
      </div>
      ...
      {/* 4. Dönem Seçici (BUGÜN / HAFTA / AY / YIL) */}
          ...
          vehicleSummaries.map((v) => (
            <div key={v.vehicle_id} style={styles.vehicleCard}>
              <div style={styles.vehicleHeader}>
                ...
                <span style={{ color: v.net_profit >= 0 ? '#22C55E' : '#F87171' }}>
                  ${v.net_profit.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </span>
              </div>
              <div style={styles.vehicleDetails}>
                <span>Gelir: ${v.total_income.toFixed(2)}</span>
                <span>Gider: ${v.total_expenses.toFixed(2)}</span>
                <span>Saat: ${v.hourly_rate.toFixed(2)}/sa</span>
                <span>Mil: ${v.per_mile_rate.toFixed(2)}/mil</span>
              </div>
            </div>
          ))
        )}
        ...
        <div style={styles.alertCard}>
          <div>
            ...
            <p style={styles.alertDesc}>
              Tutar: <strong>${notification.amount.toFixed(2)}</strong> | Kalan Süre: <strong>{notification.days_remaining} gün</strong>
            </p>
          </div>
          <button
            style={styles.payBtn}
            ...
            {dailySummaries.map((d, index) => (
              <div key={index} style={styles.dailyRow}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ color: d.net_profit >= 0 ? '#22C55E' : '#F87171', fontWeight: 'bold' }}>
                    ${d.net_profit.toFixed(2)}
                  </span>
                  <span style={styles.dailySub}>Gelir: ${d.total_income.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  tickerCard: { background: '#111827', border: '1px solid #1F2937', borderRadius: 12, padding: '10px 16px', display: 'flex', justifyContent: 'center', gap: 20, fontSize: 11, color: '#64748B', marginBottom: 20 },
};
